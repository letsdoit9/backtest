...
Usage:
    ...
    python backtester.py --full-optimization        # also runs threshold/ATR/holding/sensitivity sweeps
...
def run_backtest(min_conditions=None, max_workers=None, run_optimizations=False,
    ...
    print(f"[backtester] Generating signals (no look-ahead) using the live scanner's "
          f"exact condition logic, threshold>={config.THRESHOLD_SWEEP[0]} for optimization headroom...")
    # Generate at the LOWEST threshold in the sweep so threshold_analysis can filter
    # a strict superset without recomputing indicators/conditions repeatedly.
    base_threshold = min(config.THRESHOLD_SWEEP + [min_conditions])
    all_signals = generate_all_signals(valid_data, min_conditions=base_threshold, sector_map=sector_map)
    ...
    trades_df = execute_trades(active_signals, valid_data, target1_mult=target1_mult,
                                target2_mult=target2_mult, stop_mult=stop_mult, max_hold=max_hold)
    ...
    excel_path = write_excel_report(summary, trades_df, monthly_df, yearly_df, condition_df,
    ...
    chart_paths = generate_all_charts(equity_df, trades_df, monthly_df, sector_df, condition_df, threshold_df)
    ...
    return {
        ...
        "equity": equity_df,
        "files": {"excel": excel_path, "csv": csv_paths, "json": json_path, "charts": chart_paths},
    }
